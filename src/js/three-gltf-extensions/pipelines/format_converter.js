...
// COLOR_CODES is imported from constants.js
...
function convertPixelInfo(  height,
    ...
    if( inputFormat === 'I420' ) {
        ...
        switch( outputFormat ) {
            ...
            case 'RGB':
                ...
                colorCode = COLOR_CODES.COLOR_YUV2RGB_I420;
                break;
            case 'RGBA':
                outputBytes = height * width * 4;
                colorCode = COLOR_CODES.COLOR_YUV2RGBA_I420;
                break;
            case 'BGR':
                outputBytes = height * width * 3;
                colorCode = COLOR_CODES.COLOR_YUV2BGR_I420;
                break;
            case 'BGRA':
                outputBytes = height * width * 4;
                colorCode = COLOR_CODES.COLOR_YUV2BGRA_I420;
                break;
            case 'DEPTH_COMPONENT':
                ...
                colorCode = COLOR_CODES.COLOR_YUV2GRAY_420;
                break;
            default:
    ...
    // handle cases where input format is 'I422'
        ...
        switch( outputFormat ) {
            ...
            case 'RGB':
                outputBytes = height * width * 3;
                colorCode = COLOR_CODES.COLOR_YUV2RGB_Y422;
                break;
            case 'RGBA':
                outputBytes = height * width * 4;
                colorCode = COLOR_CODES.COLOR_YUV2RGBA_Y422;
                break;
            case 'BGR':
                outputBytes = height * width * 3;
                colorCode = COLOR_CODES.COLOR_YUV2BGR_Y422;
                break;
            case 'BGRA':
                outputBytes = height * width * 4;
                colorCode = COLOR_CODES.COLOR_YUV2BGRA_Y422;
                break;
            case 'DEPTH_COMPONENT':
                // TODO: we'll convert to GRAY here, but really that should
                // not be the case. probably need to support
                // a single channel with >8 bit depth
                outputBytes = height * width;
                colorCode = COLOR_CODES.COLOR_YUV2GRAY_Y422;
                break;
            default:
                // convert to RGB by default
                outputBytes = height * width * 3;
    ...
    // handle cases where input format is 'NV12'
        ...
        switch( outputFormat ) {
            ...
            case 'RGB':
                outputBytes = height * width * 3;
                colorCode = COLOR_CODES.COLOR_YUV2RGB_NV12;
                break;
            case 'RGBA':
                outputBytes = height * width * 4;
                colorCode = COLOR_CODES.COLOR_YUV2RGBA_NV12;
                break;
            case 'BGR':
                outputBytes = height * width * 3;
                colorCode = COLOR_CODES.COLOR_YUV2BGR_NV12;
                break;
            case 'BGRA':
                outputBytes = height * width * 4;
                colorCode = COLOR_CODES.COLOR_YUV2BGRA_NV12;
                break;
            case 'DEPTH_COMPONENT':
                // TODO: we'll convert to GRAY here, but really that should
                // not be the case. probably need to support
                // a single channel with >8 bit depth
                outputBytes = height * width;
                colorCode = COLOR_CODES.COLOR_YUV2GRAY_NV12;
                break;
            default:
                // convert to RGB by default
                outputBytes = height * width * 3;
    ...
    // for our purposes, 'RGBX' is the same as 'RGBA'
        ...
        switch( outputFormat ) {
            ...
            case 'RGB':
                outputBytes = height * width * 3;
                colorCode = COLOR_CODES.COLOR_RGBA2RGB;
                break;
            case 'RGBA':
                outputBytes = height * width * 4;
            ...
            case 'BGR':
                outputBytes = height * width * 3;
                colorCode = COLOR_CODES.COLOR_RGBA2BGR;
                break;
            case 'BGRA':
                outputBytes = height * width * 4;
                colorCode = COLOR_CODES.COLOR_RGBA2BGRA;
                break;
            case 'DEPTH_COMPONENT':
                // TODO: we'll convert to GRAY here, but really that should
                // not be the case. probably need to support
                // a single channel with >8 bit depth
                outputBytes = height * width;
                colorCode = COLOR_CODES.COLOR_RGBA2GRAY;
                break;
            default:
                // convert to RGB by default
                outputBytes = height * width * 3;
    ...
    // for our purposes, 'BGRX' is the same as 'RGBA'
        ...
        switch( outputFormat ) {
            ...
            case 'RGB':
                outputBytes = height * width * 3;
                colorCode = COLOR_CODES.COLOR_BGRA2RGB;
                break;
            case 'RGBA':
                outputBytes = height * width * 4;
                colorCode = COLOR_CODES.COLOR_BGRA2RGBA;
                break;
            case 'BGR':
                outputBytes = height * width * 3;
                colorCode = COLOR_CODES.COLOR_BGRA2BGR;
                break;
            case 'BGRA':
                outputBytes = height * width * 4;
                // leave colorCode 'null'. no conversion needed!
            ...
            case 'DEPTH_COMPONENT':
                ...
                // a single channel with >8 bit depth
                outputBytes = height * width;
                colorCode = COLOR_CODES.COLOR_BGRA2GRAY;
                break;
            default:
                // convert to RGB by default
                outputBytes = height * width * 3;
